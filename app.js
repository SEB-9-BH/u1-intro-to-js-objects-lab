...
*/
game.party.push(pokemon[5],pokemon[39],pokemon[54])
...
game.catchPokemon = function (pokemonName) {
     ...
     findPokemon = function (pokemonName){for (i=0;i<pokemon.length;i++){
    ...
    const tempPokemon = findPokemon(pokemonName)
    if(tempPokemon){
    if(game.items[1].quantity<1){
        console.log("there are not enough pokeballs to catch the desired Pokemon.")
    }else if( game.party.length<6){
    game.items[1].quantity-=1    
    game.party.push(tempPokemon)}
    else {
        game.collection.push(tempPokemon)
    }
  }else {
...
const pokemonTypelist = {};
  for (let i = 0; i < pokemon.length; i++) {
    ...
    if (pokemonTypelist[type]) {
      pokemonTypelist[type].push(pokemon[i])
    } else {
       pokemonTypelist[type]=[pokemon[i]]
    }
  }
